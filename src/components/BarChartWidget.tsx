...
export default function BarChartWidget({ data, config, isSelected, onRemove, onSelect }: BarChartWidgetProps) {
  const processedData = useMemo(() => {
    ...
    return [
      ...
      ...aggregated.map(row => [row[groupKey] ?? '', ...config.yKeys.map(key => row[key] ?? null)])
    ];
  ...
  const options = {
    ...
    series: config.yKeys.reduce((acc, key, index) => {
      acc[index] = { color: config.columnColors?.[key] || '#6366f1' };
      return acc;
    }, {} as any),
