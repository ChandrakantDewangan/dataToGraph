...
export default function AreaChartWidget({ data, config, isSelected, onRemove, onSelect }: AreaChartWidgetProps) {
  ...
  const options = {
    ...
    areaOpacity: config.opacity || 0.3,
    series: config.yKeys.reduce((acc, key, index) => {
      acc[index] = { color: config.columnColors?.[key] || '#ec4899' };
      return acc;
    }, {} as any),
